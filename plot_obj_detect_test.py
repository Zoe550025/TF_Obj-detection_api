...
def inference_with_plot(path2images,fname,root_path,box_th=0.25):
    ...
    Args:
      ...
      root_path： a path which images are stored.

    Returns:
      None
    """
    for image_path,image_name in zip(path2images,fname):
        ...
        plt.figure(figsize=(8.32,8.32),dpi=100)
        plt.imshow(image_np_with_detections)
        plt.axis('off')
        print('Done')
        plt.savefig(root_path+image_name, bbox_inches='tight',pad_inches = 0,dpi=100)
        plt.close()

        filename_string = root_path+"coordinates"
...
stored_path = "E:\\tzuwen\\tree_segmentation2\\resize_test\\640_test_0413\\results\\ "#"E:\\tzuwen\\tree_segmentation2\\resize_test\\640\\others\\2\\2-9\\results\\"
path = "E:\\tzuwen\\tree_segmentation2\\resize_test\\640_test_0413"#"E:\\tzuwen\\tree_segmentation2\\resize_test\\640\\others\\2\\2-9"
pic_lis = []
pic_name = []

for p in Path(path).iterdir():
    if p.is_file():
        pic_lis.append(str(p))
        pic_name.append(str(p)[-19:])
#print(pic_name)
inference_with_plot(pic_lis,pic_name,stored_path)
